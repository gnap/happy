...
export function parseCursorMessage(msg: CursorStreamMessage): CursorParsedMessage[] {
  ...
  switch (msg.type) {
    case 'system': {
      ...
      // Do NOT emit task_started here - the main loop handles turn lifecycle
      break;
    }
    ...
    case 'result': {
      // Only emit task_complete with metadata - do NOT emit text_delta here
      // because the same text was already emitted by the preceding 'assistant' message.
      // Emitting it again would cause duplicate text on the mobile app.
      results.push({
        type: 'task_complete',
        sessionId: msg.session_id,
