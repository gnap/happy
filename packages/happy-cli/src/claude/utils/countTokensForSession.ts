 ...
 *
 * Returns null on any failure (missing auth, network error, empty session).
 */
export async function countTokensForSession(opts: {
    ...
    if (!authToken) {
        logger.debug('[countTokens] No ANTHROPIC_AUTH_TOKEN, skipping');
        return null;
    }
    ...
    try {
        ...
        if (!res.ok) {
            logger.debug(`[countTokens] API error ${res.status} for session ${opts.sessionId}`);
            return null;
        }
