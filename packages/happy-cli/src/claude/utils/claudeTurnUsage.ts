 ...
 * rollup, and treat the running prompt size as
 * `cacheRead + cacheCreation + freshInput` (the same components that bill against the
 * model's context window each turn).
 */
...
export function buildClaudeTurnUsagePayload(
    result: Pick<SDKResultMessage, 'usage' | 'modelUsage'>,
    ...
    const contextSize = (cacheRead ?? 0) + (cacheCreate ?? 0) + (inputTokens ?? 0);
    ...
    return {
        ...
        context_size: contextSize > 0 ? contextSize : undefined,
