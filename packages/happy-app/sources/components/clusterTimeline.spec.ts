...
describe('computeMessageClusters', () => {
    ...
    describe('multi-turn segments', () => {
        ...
        it('shows all tasks in one cluster per segment', () => {
            const msgs: Message[] = [
        ...
        it('matches by fuzzy content when subject differs from result text', () => {
            ...
            expect(cards[0].tasks[0].status).toBe('completed');
            expect(cards[0].tasks[1].status).toBe('completed');
            expect(cards[0].tasks[2].status).toBe('completed'); // fuzzy content match
        });

        it('merges all task batches into one cluster per segment', () => {
            const msgs: any[] = [];
        ...
        it('completes tasks in multi-batch without session.tasks (real session sim)', () => {
            ...
            // One cluster per segment: all 8 tasks in one card
            expect(cards.length).toBe(1);
            expect(cards[0].tasks.length).toBe(8);
            // Task at index 0: taskId 33, completed
            expect(cards[0].tasks[0].status).toBe('completed');
            // Task at index 5 (C1): taskId 38, in_progress (last TaskUpdate was in_progress)
            expect(cards[0].tasks[5].status).toBe('in_progress');
            // Tasks at index 6, 7: completed
            expect(cards[0].tasks[6].status).toBe('completed');
            expect(cards[0].tasks[7].status).toBe('completed');
        });
