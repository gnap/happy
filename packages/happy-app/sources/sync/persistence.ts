...
export type NewSessionAgentType = 'claude' | 'codex' | 'cursor' | 'cursor-acp' | 'gemini';
...
export function loadNewSessionDraft(): NewSessionDraft | null {
    ...
    try {
        ...
        const agentType: NewSessionAgentType = parsed.agentType === 'codex' || parsed.agentType === 'cursor' || parsed.agentType === 'cursor-acp' || parsed.agentType === 'gemini'
            ? parsed.agentType
            : 'claude';
...
/** Session id -> model mode key. Fallback when session.metadata.currentModelCode is absent. */
